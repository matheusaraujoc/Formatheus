# dialogo_3dchart.py
#
# Descrição: Versão de teste standalone do editor de gráficos 3D.
#
# ATUALIZAÇÃO (v62 - Alternador de Tema):
# 1. Adicionado botão discreto (🌗) no topo direito para alternar
#    entre temas Claro e Escuro (requer 'qdarktheme').
# 2. Mantém todas as funcionalidades anteriores (Importação, 
#    Modo Grade/Pontos, Tipos 3D).
#
...
# Tenta importar o tema, se não existir, a funcionalidade é desativada
try:
    import qdarktheme
    HAS_THEME_LIB = True
except ImportError:
...
# --- CLASSE GRAFICO (Simulada para Teste) ---
class Grafico:
    def __init__(self):
        self.titulo = ""
        self.fonte = ""
        self.caminho_imagem_processada = ""
        self.caminho_dados_json = ""
        self.largura_cm = 16.0
...
class SettingsPanel(QWidget):
    ...
    def _create_geral_tab(self):
        ...
        self.source_input = QLineEdit("Fonte: Teste")
...
class ChartDialog3D(QDialog):
    ...
    def __init__(self, parent: QWidget = None):
        ...
        self.grafico_final = Grafico()
        self.banco_graficos = []
        self.grafico_original_para_edicao = None
        self.is_dark_theme = True # Estado inicial

        main_layout = QVBoxLayout(self) 
        
        # --- BARRA SUPERIOR (Botão de Tema) ---
        top_bar = QHBoxLayout()
        top_bar.addStretch()
        ...
        # Painel 1: Stack de Dados (Esquerda)
        ...
        # Adiciona dados de teste iniciais
    ...
    def _plot_points(self, ax, s):
        ...
        for i, series in enumerate(series_list):
            ...
            elif plot_type == 'Arame (Wireframe)': # Fallback para triangulação
                 if len(x) >= 3:
                    try:
    ...
    def _plot_grid(self, ax, data_state, s):
        ...
        if plot_type == 'Superfície de Grade':
            ...
            if 'cmap' in plot_kwargs: del plot_kwargs['cmap']; plot_kwargs['color'] = color # Simplifica
    ...
    def accept(self):
        ...
        if not novo_titulo:
            QMessageBox.warning(self, "Campo Obrigatório", "O 'Título' não pode estar vazio.")
            return
        ...
        caminho_imagem_png = os.path.join(pasta_dados_chart, f"{nome_arquivo_base}.png")
        ...
        try:
            ...
            QMessageBox.information(self, "Salvo com Sucesso",
                f"Arquivos de teste salvos em:\n{os.path.abspath(pasta_dados_chart)}")
