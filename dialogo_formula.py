...
# ATUALIZAÇÃO: Adicionada verificação de legenda duplicada
# no método trigger_save_process() para evitar bugs de referência.
...
class DialogoFormula(QDialog):
    ...
    # --- INÍCIO DA MODIFICAÇÃO (Adiciona banco_formulas) ---
    def __init__(self, formula: Formula = None, banco_formulas: list[Formula] = None, parent: QWidget = None):
    # --- FIM DA MODIFICAÇÃO ---
        ...
        self.formula_original_para_edicao = formula
        self.formula = formula if formula else Formula()
        self.banco_formulas = banco_formulas if banco_formulas else []
        # --- FIM DA MODIFICAÇÃO ---
    ...
    def trigger_save_process(self):
        ...
        # --- INÍCIO DA MODIFICAÇÃO (Validação e Sincronização) ---
        nova_legenda = self.legenda_input.text().strip()
        
        if not nova_legenda:
            QMessageBox.warning(self, "Dados Incompletos", "A legenda da fórmula é obrigatória.")
            return # Não continua

        # Compara com todas as formulas no banco
        if self.banco_formulas: # Garante que a lista não é None
            for form_existente in self.banco_formulas:
                # 1. Compara a legenda
                if form_existente.legenda.strip().lower() == nova_legenda.lower():
                    
                    # 2. Verifica se é a MESMA formula
                    if self.formula_original_para_edicao is form_existente:
                        continue # É o mesmo objeto, permite salvar.

                    # 3. Se for um objeto DIFERENTE com o mesmo nome, bloqueia.
                    QMessageBox.warning(self, "Legenda Duplicada", 
                                        f"Já existe uma fórmula com a legenda '{nova_legenda}'.\n"
                                        "A legenda da fórmula deve ser única.")
                    return # Não fecha o diálogo
        
        # Atualiza o objeto self.formula AGORA, antes de salvar.
        self.formula.legenda = nova_legenda
    ...
    def get_dados_formula(self) -> Formula:
        # Os dados (legenda, largura, latex, caminhos) 
        # já foram atualizados em self.formula
        # dentro de trigger_save_process() e _on_download_state_changed()
        # Esta função apenas retorna o objeto já atualizado.
        return self.formula